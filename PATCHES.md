...
## 2b. P10 — 思考面板默认展开（流式实时可见）

**产品语义（2026-09-19 用户澄清）**：「思考过程显示」= 执行中直接看到推理内容，不是折叠条。
原应用思考折叠面板 `useState(!1)` 默认收起，每条消息都要手动点开。

**变换（1 处）**：c3 思考组件的首个 `useState(!1)` → `useState(!0)`。
- 流式中的消息：思考内容实时可见（配合尾部流式渲染点）
- 完成态：思考块被 P1 整体隐藏，不受影响
- 实现于 `patch_thinking_live()`：独立于 MARKER 幂等（闭式锚点 useState(!0) 自消失）
