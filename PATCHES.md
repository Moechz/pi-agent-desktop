...
**七调（用户 2026-09-21）：+ 号改小改细**
“+ 新会话”的加号太粗大、与文字不协调 → 11px 画框不变（上轮改 13 偏大，回退）、
描边 `2.2→1.6`、十字从满框（1..11）收进到 **2.5..9.5**（视觉宽约 6.4px，与 13px 文字匹配）。
⚠ `strokeWidth:"1.6"` 原版已有 2 处，不能当自检标记；用十字坐标 `{x1:"6",y1:"2.5",x2:"6",y2:"9.5"}`
（原版 0 处）作唯一标记。想再微调：改十字起止值（越小越短）与 strokeWidth。
...
## 4h. P18 — 侧边栏「资源管理器」默认收起（用户 2026-09-20）

**需求**：应用重启后左侧栏底部「资源管理器」（EXPLORER）区块默认收起，不要默认展开。

**实现**：X 组件里 explorer 展开状态声明 `p??null,[T,A]=(0,r.useState)(!0),`（锚点已验证唯一；
T 同时控制区块高度 flex、chevron rotate(90deg)、面板显隐，与 h("sidebar.explorer") 同域）。
仅把默认值 !0 → `/*__piExpl*/!1`（块注释当自检标记，合法 JS）；点击展开/收起逻辑不变。
...
## 4j. P20 — 字号五档制（文字体系统一，2026-09-20）

**需求**：用户反馈前端 UI 及设置菜单各处文字大小不统一，要求制定
标题/正文/提示/注释几类使用场景并收敛种类。

**审计**（原版 chunk 内联 fontSize）：10×16、11×53、12×78、13×18、14×6、15×2、
9×2、12.5×1、3.4×1，同层内容随机落在不同档位。

**规范（档位 + 主题四级灰，颜色按重要性选不按档位）**：

| 档 | 字号 | 字重/颜色惯例 | 场景 |
|---|---|---|---|
| 页标题 | 15 | w700 / text | 设置页大标题 |
| 区块标题 | 14 | w600 / text | 设置分区、面板标题 |
| 正文 | 13 | 400~600 / text~strong | 菜单项、列表行、按钮、输入、弹窗行 |
| 提示 | 12 | 400 / muted | 副文本、tab 标签、快捷键、meta、代码块(12.5→12) |
| 注释 | 11 | 400~600 / dim | 分组小标题(UPPERCASE)、时间戳、徽标/角标、空状态、hint |

特例保留：20（空状态大图标）、16（密钥展示 mono w700）、3.4（SVG 内部字形，非 UI 文字）。
语义色（danger/success/warning/accent）只管颜色不管档位。

**实现**：patch_typography()，在补丁链最后跑（P8c 锚点依赖原始字号值，必须
其后）：10→11、9→11、9.5/10.5→11（P15/P16 自注的边角）、12.5→12，共 21 处；
幂等=无残留即跳过。与 P8c（12→13、11→12 全局升档）叠加后给到最终五档：
