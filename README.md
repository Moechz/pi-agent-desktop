...
| P20 | 字号五档制 | 全 UI 文字收敛为 11 注释/12 提示/13 正文/14 区块标题/15 页标题五档（扫 10/9/9.5/10.5→11、12.5→12 共 21 处）；颜色沿用主题四级灰 strong/text/muted/dim 按重要性选用；P17 弹窗标题改 strong 色、行归 13 |
| P21 | 中英文统一 | 审计：locale 字典完整，问题在主 chunk 硬编码英文；22 处 UI 文字译入中文（空/加载中…系列/默认·禁用·自定义/OAuth 登录流程三句/思考兼容/例如系列占位符/OpenAI·Anthropic 兼容等）；品牌、协议名、TSX/SQL 缩写保留；条件比较假阳性已甄别 |
