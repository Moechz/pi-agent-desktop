...
| P16 | 顶部路径栏 → “+ 新目录” | 侧边栏顶部不再显示当前工作目录路径，改为「闭合文件夹+加号」方形图标按钮：点击直通系统文件夹选择器（原下拉里“+ 自定义路径”的功能提到最外层）；右侧小 ▾ 保留原历史目录下拉（最近 5 个/默认目录/自定义路径）入口；主钮方形 28px，四钮同高；原右上「新会话+刷新」按钮组下移至同一行右侧 |
