...
| P6 | 菜单弹窗不透明无色偏 | 所有菜单/弹窗（右键菜单、模型下拉、对话框等 material-popover 系）背景引用 var(--bg)：完全跟随主题基础色（暗=近黑、亮=近白），不透明、无色偏、明暗自适应，靠边框+阴影区分层次（原暗色 90%/亮色 86% 半透明 + 30px 模糊） |
| P7 | 输入框下方图标加大 | 工具行图标 15→18/14→17/13→16/11→14px（附件、模型选择器、模式切换、工具预设、更多控件），看清不少 |
