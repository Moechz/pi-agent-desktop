...
| P15 | 会话条目紧凑化 | 删去标题下方 meta 行（时间+消息数，排版乱）；标题行最右固定位置显示紧凑相对时间（now/Nm/Nh/Nd，数字等宽不跳动，悬停看完整时间戳）；取消空闲灰点，只保留运行中绿点；行高 52→40px（单行紧凑）；组头图标改文件夹符号（收起=关闭文件夹/accent，展开=打开文件夹/muted，点击仍可折叠展开） |
