...
| P17 | 新会话输入框目录名 + ▾ 切换目录 | 点“+ 新会话”后输入框左上角显当前目录名（末段，hover 全路径），▾ 弹窗切换：「已添加目录」全量列表（每行文件夹 icon + 目录名，当前项行尾打勾，340px 内滚动）/ 使用默认目录 / 选择其他目录…；目录清单为弹窗打开时自 fetch /api/sessions 计算；复用 ej(id,cwd)（与侧边栏“+ 新会话”同函数，目录在第 2 参）；仅新会话态显示。📄 详 PATCHES.md §4g |
